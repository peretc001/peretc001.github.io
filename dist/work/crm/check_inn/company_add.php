...
<body>
	...
	<section class="company_page">
		<div class="container-fluid">
			<form action="" method="post" class="save_form">
				<div class="row company_page_menu">
					<button class="btn btn-success save_btn" disabled>Сохранить</button>
					...
					<div class="col-lg-4">
						<div class="company_page_block">
							...
							<div class="form-group row">
								...
								<div class="col-sm-9">
									<input id="company_code" type="tel" name="company_code" class="form-control ecj" value="" placeholder="ИНН" maxlength="12" required />
									<span id="company_desc" style="display:none;background:#ff0000;border-radius:4px;padding:0.3rem 0.5rem;color:#fff;font-size:0.8em;">Допустимо 10 или 12 знаков</span>
								</div>
							</div>
							<div class="form-group row">
	...
	<script src="/js/uikit/jquery-ui.js"></script>
	<script>
		const form 			= document.querySelector('.save_form');
		const btn 			= document.querySelector('.save_btn');
		const company_name 	= document.querySelector('#company_name');
		const company_code 	= document.querySelector('#company_code');
		const company_desc 	= document.querySelector('#company_desc');
		company_code.addEventListener('keyup', function() {
			const codeVal = company_code.value.length;
			const nameVal = company_name.value.length;

			btn.disabled = true;
			
			if(codeVal === 10 || codeVal === 12) {
				btn.disabled = false;
				form.action = '/inc/action/add_company.php';
				company_desc.style.display = 'none';
			} else {
				btn.disabled = true;
				form.action = '';
				company_desc.style.display = 'inline';
			}
		});
		$(function(){
			$("#company_code").autocomplete({
				source: '/inc/action/company_list.php',
				minLength: 6,
			}).data('ui-autocomplete')._renderItem = function(ul, item) {
			        return $( "<li></li>" ).data("item.autocomplete", item)
			            .append( '<a href="/company_page.php?id=' + item.desc + '">' + item.label + '</a>')
			            .appendTo( ul );
			};
		});
	</script>